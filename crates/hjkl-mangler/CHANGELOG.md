...
- `formatter_for_path(&Path) -> Option<Box<dyn Formatter>>` — dispatches by
  file extension across 8 built-in tools (rustfmt, prettier, gofmt, ruff,
  stylua, shfmt, taplo, black).
...
- `FormatWorker` — async per-buffer format dispatch with stale-result drop
  via `dirty_gen` and per-`BufferId` dedup.
- `is_tool_installed(tool)` / `probe_tool(tool)` — host-side availability
  checks. `is_tool_installed` uses spawn-success (compatible with shells
  like dash that don't grok `--version`); `probe_tool` runs `--version` and
  returns exit-code diagnostics.
- `FORMAT_TIMEOUT = 30s` — covers large-file rustfmt runs without
  prematurely killing the child.
- `StdinFormatter`, `PrettierFormatter`, `RustFormatter`, `StyluaFormatter`,
  ...
  background threads before writing stdin (required for outputs above the
  64 KiB pipe buffer).
