...
mod tests {
    ...
    fn xdg_empty_falls_back_to_home() {
        ...
        let r = resolve_xdg(Some(&env), Some(home.path()), ".config").unwrap();
        assert_eq!(r, home.path().join(".config"));
    }

    #[test]
    fn xdg_unset_falls_back_to_home() {
        let home = tempfile::tempdir().unwrap();
        let r = resolve_xdg(None, Some(home.path()), ".local/share").unwrap();
        assert_eq!(r, home.path().join(".local/share"));
    }
    ...
    fn xdg_data_home_ignores_relative_path() {
        let env = os("relative");
        let home = tempfile::tempdir().unwrap();
        let r = resolve_xdg(Some(&env), Some(home.path()), ".local/share").unwrap();
        assert_eq!(r, home.path().join(".local/share"));
    }
    ...
    fn xdg_config_home_falls_back_when_unset() {
        let home = tempfile::tempdir().unwrap();
        let r = resolve_xdg(None, Some(home.path()), ".config").unwrap();
        assert_eq!(r, home.path().join(".config"));
    }
    ...
    fn xdg_cache_home_falls_back_when_unset() {
        let home = tempfile::tempdir().unwrap();
        let r = resolve_xdg(None, Some(home.path()), ".cache").unwrap();
        assert_eq!(r, home.path().join(".cache"));
    }
    ...
    fn xdg_data_dir_appends_app() {
        ...
        assert_eq!(base.join("bar"), tmp.path().join("bar"));
    }
