...
pub struct PreviewSpans {
    ...
    /// Style id → engine style. Index with the `style` field of each
    /// `BufferSpan`.
...
impl PreviewSpans {
    ...
    pub fn from_byte_ranges(
        ranges: &[(std::ops::Range<usize>, EngineStyle)],
        bytes: &[u8],
    ) -> Self {
        ...
        let mut styles: Vec<EngineStyle> = Vec::new();
