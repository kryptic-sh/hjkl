...
impl PickerLogic for FileSource {
    ...
    fn preview_path(&self, idx: usize) -> Option<PathBuf> {
        let rel = self.items.lock().ok()?.get(idx).cloned()?;
        Some(self.root.join(rel))
    }

    fn select(&self, _idx: usize) -> PickerAction {
        // FileSource is always wrapped by an app-side source that
        // overrides `select` and boxes an app-specific `AppAction`.
        // This base impl is never called directly.
        PickerAction::None
    }
...
mod tests {
    ...
    #[test]
    fn txt_preview_returns_buffer_and_path() {
        ...
        let (_buf, status) = source.preview(idx);
