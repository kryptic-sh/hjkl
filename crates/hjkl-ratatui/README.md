...
ratatui `Style` ↔ engine `Style` adapter; `crossterm::KeyEvent` → engine `Input`
bridge. Includes a `spinner` module providing a shared braille loading indicator
for use across TUI widgets.
...
- `crossterm_key_event_to_input` (behind the default-on `crossterm` feature) —
  bridge `crossterm::KeyEvent` → engine `Input`.
