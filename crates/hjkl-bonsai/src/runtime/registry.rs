...
mod tests {
    ...
    #[test]
    fn embedded_neovim_literal_extension_aliases() {
        let r = embedded();
        let aliases = [
            ("F08", "fortran"),
            ("F77", "fortran"),
        ...
        for (extension, language) in aliases {
            assert_eq!(
                r.name_for_ext(extension),
                Some(language),
                ".{extension} should resolve to {language}"
            );
        }

        assert_eq!(r.name_for_path(&PathBuf::from("source.c")), Some("c"));
        assert_eq!(r.name_for_path(&PathBuf::from("source.C")), Some("cpp"));
    }
}
