...
//!
//! let dir = LanguageDirectory::new().expect("language directory");
//! // An unrecognised extension returns Unknown without blocking.
//! assert!(matches!(
//!     dir.request_for_path(&PathBuf::from("hello.zzznope")),
...
pub enum GrammarRequest {
    ...
    /// No spec / no recognised extension. Plain-text only for this buffer.
    Unknown,
}
...
/// use std::path::PathBuf;
///
/// let dir = LanguageDirectory::new().expect("init");
/// match dir.request_for_path(&PathBuf::from("main.rs")) {
///     GrammarRequest::Unknown => {}     // fine on a cold system
///     GrammarRequest::Cached(_) => {}   // grammar already installed
///     GrammarRequest::Loading { .. } => {} // clone+compile kicked off
///     _ => {}                            // future variants
/// }
/// ```
...
mod tests {
    ...
    #[test]
    fn request_for_path_with_txt_returns_unknown() {
        let dir = LanguageDirectory::new().unwrap();
        // .txt has no grammar spec in bonsai.toml.
        let result = dir.request_for_path(&PathBuf::from("README.txt"));
        assert!(matches!(result, GrammarRequest::Unknown));
    }

    #[test]
    fn multiple_unknown_requests_do_not_panic() {
        let dir = LanguageDirectory::new().unwrap();
        for _ in 0..10 {
