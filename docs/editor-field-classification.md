...
| `styled_spans` (ratatui feature)   | 624          | `Vec<Vec<(usize, usize, ratatui::style::Style)>>` | per-buffer        | Buffer (or Editor)                                              | syntax spans are a property of buffer content; however they are re-installed per parse and currently tied to an editor — see ambiguous decisions |
...
| `buffer_spans`                     | 699          | `Vec<Vec<hjkl_buffer::Span>>`                     | per-buffer        | Buffer (or Editor)                                              | syntax spans derive from buffer content — see ambiguous decisions                                                                                |
...
## Per-window field count: 40

All VimState fields (35) plus `keybinding_mode`, `vim` (the VimState itself),
