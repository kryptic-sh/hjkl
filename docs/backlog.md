...
## 9. 2026-08-06 review (whole-codebase correctness sweep)

Three findings, all verified by re-tracing the cited lines (see "Verification"
per finding). Clean tree on `main` → whole-codebase scope, low depth.

### Findings — ranked

#### 1. MEDIUM — ex addresses past EOF clamp silently; `:5d` on a 3-line file deletes the last line (nvim errors E16)

`crates/hjkl-ex/src/range.rs:196` resolves a numeric address with
`.clamp(1, last)` instead of rejecting it, so an out-of-range line number
silently becomes the last line. The delete handler
(`crates/hjkl-ex/src/builtins.rs:881` → `resolve_range_with_count` at
`builtins.rs:687`, which clamps `end_row` to `total-1`) then proceeds and
deletes. nvim rejects `:5d` on a 3-line buffer with "E16: Invalid range" and
deletes nothing.

Verification: `range.rs:196` (`clamp(1, last as i64)`); `builtins.rs:698-699`
and `:707-719` (`start_row`/`end_row` derived with `min(total-1)`); delete loop
at `builtins.rs:896-917` removes rows `start_row..=end_row`. The sub-agent
reported live-verification against both nvim and the hjkl binary.

```
Repro: 3-line buffer, run `:5d`
Expect: E16 "Invalid range", nothing deleted (nvim behaviour)
Actual: last line silently deleted
```

#### 2. MEDIUM — `:retab` panics (divide by zero) after a `# vim: tabstop=0` modeline

`crates/hjkl-ex/src/builtins.rs:3412` uses `editor.settings().tabstop`
unguarded. With `tabstop == 0`, `convert_whitespace` computes `vcol % tabstop`
(`builtins.rs:3205` expandtab path, `:3229` non-expandtab path) — `% 0` panics
("attempt to calculate the remainder with a divisor of zero"). The `:3253`
`is_multiple_of(tabstop)` is safe (`is_multiple_of(0)` returns `false`, verified
by compiling a probe). `tabstop=0` is reachable: modeline `ts=0` passes
`Options::set_by_name` → `set_u32!` which accepts 0
(`crates/hjkl-engine/src/types.rs:578`). Every sibling consumer normalizes with
`.max(1)`: `crates/hjkl-vim/src/vim/text_object_ops.rs:375`, `command.rs:613`,
`hjkl-buffer/src/listchars.rs:187`, `apps/hjkl/src/render.rs:1093`. The explicit
`:retab 0` arg IS guarded (`builtins.rs:3402-3405`); the settings-sourced path
is not. Reached end-to-end from file open: `build_slot` applies the modeline
overlay (`apps/hjkl/src/app/mod.rs:905-910`).

```
Repro: file whose first line is `# vim: ts=0`, with a line of leading tabs,
       then `:retab`
Expect: no crash (or "tabstop must be > 0")
Actual: panic "attempt to calculate the remainder with a divisor of zero"
```

#### 3. LOW — `hjkl-tabs` `TabBar::close` moves focus when a non-active tab is closed

`crates/hjkl-tabs/src/lib.rs:220-230`: `close` unconditionally sets
`self.active = Some(pos.min(len-1))`, even when the removed tab was not the
active one. Closing a non-active tab should leave focus on the current tab
(adjusted only for the index shift); the code instead focuses the tab at the
removed position. The doc example (`lib.rs:215-218`) only documents closing the
active tab, so the contract is vague, but moving focus on a non-active close
contradicts the "close this tab, keep my place" expectation of a tab bar.
Unreachable from hjkl's own UI today (no `TabBar::close` callers in `apps/hjkl`
or `crates/hjkl-tabs-tui`), but the crate is published.

```
Repro: tabs [1,2,3], active = 1; close(&3)
Expect: active stays 1 (index 0)
Actual: active becomes Some(1) → tab 2 (index 1)
```

### Cleared

- `is_multiple_of(0)` does not panic (returns `false`) — only the two `%` sites
  in `convert_whitespace` are the panic; verified by compiling a probe.
- `:retab 0` explicit arg is rejected ("tabstop must be > 0",
  `builtins.rs:3402-3405`).
- Backward ranges (`:8,3d`) already error with E493 (`range.rs:344-352`).
- `:set ts=0` is rejected by the registry setter (min=1,
  `options_registry.rs:161-166`) — only the modeline path bypasses it.
- Modeline token validation (`modeline.rs:129-131`) probes `set_by_name`,
  dropping unknown/type-mismatched options.

### Hardening

- `:retab`'s `tabstop` (and the same class of "settings-sourced divisor"
  consumers elsewhere) should normalize with `.max(1)` like every sibling, so a
  future option-range loosening cannot reintroduce the panic.
- `range.rs:196` clamping is deliberate for search/`$` addresses; a numeric
  literal address past EOF is where vim's E16 is expected — consider
  distinguishing literal numbers from derived addresses.

### Coverage

Verified in full for each finding above (cited lines re-read and traced). The
rest of the ~262k-LOC workspace was not re-reviewed in this pass beyond what the
audit (section 10) and the prior §8 review covered; the sub-agent's broader skim
found no other high-confidence correctness findings. Platform-gated code
(`crates/hjkl-clipboard` windows.rs / macos.rs) not compiled on this host.

## 10. 2026-08-06 audit (whole-codebase security sweep)

Clean tree on `main` → whole-codebase scope, low depth. Findings verified by
re-tracing the cited lines from entry point to impact.

### Findings — ranked

#### 1. HIGH — nvim-api RPC `:e`/`:cfile` read arbitrary paths, and `:cd` moves the confinement root; the RPC fs-confinement is bypassable

`apps/hjkl/src/main.rs:536-538` enables `policy::restrict_fs()` for both
`--embed` and `--nvim-api`, intending `:w`/`:e`/`:r` to be confined to the
working-directory subtree. But the app-layer `:e` path never consults the
policy: `nvim_command` (`apps/hjkl/src/nvim_api.rs:1144`) → `app.dispatch_ex` →
`ExEffect::EditFile` (`apps/hjkl/src/app/ex_dispatch.rs:520`) → `do_edit`
(`ex_dispatch.rs:2134`) → `open_new_slot`
(`apps/hjkl/src/app/buffer_ops.rs:594`) → `build_slot`
(`apps/hjkl/src/app/mod.rs:868`) reads via `hjkl_fs::read_to_string_unbounded`
with **no** `check_fs_path`/`resolve_under`. Same for `:cfile`:
`crates/hjkl-ex/src/builtins.rs:2834` → `QfCommand::FromFile` →
`apps/hjkl/src/app/quickfix.rs:906` does a bare `std::fs::read_to_string`.
`check_fs_path` is only wired into `:r` (`builtins.rs:169-184`) and the write
path (`apps/hjkl/src/save.rs:197-217`) — the read paths were missed.

The write path's confinement is also movable: `:cd <dir>` calls
`std::env::set_current_dir` with no gate (`crates/hjkl-ex/src/builtins.rs:532`),
so a client can `:cd /` and the `resolve_under(cwd, …)` root for `:w`
(`save.rs:205`, cwd taken at `ex_dispatch.rs:1283`) becomes `/` — then relative
writes escape anywhere. `:cd` is registered in the default registry
(`builtins.rs:1774`) and reachable from `nvim_command`.

Contrast: the embed-mode RPC (`apps/hjkl/src/embed.rs:204-224`) DOES gate the
same `ExEffect::EditFile` with `check_fs_path` + `resolve_under` — so the policy
exists, is correct, and is enforced on one of the two RPC entry points and not
the other.

Traced exploit: client sends `nvim_command(":e /etc/shadow")` (or
`:cfile /etc/shadow`, or `:cd /` then `:w etc/cron.d/x`) over the msgpack-rpc
channel. `nvim_command` passes the string through unvalidated
(`nvim_api.rs:1139-1144`); `:e` reads the file into a buffer the client can then
read via `nvim_buf_get_lines`. Read of any user-readable file, write of any
user-writable path.

```
Repro: nvim_command(":e /etc/hostname") then nvim_buf_get_lines on the new buffer
Expect: "path /etc/hostname is outside the working directory (blocked in RPC mode)"
Actual: file contents returned
```

Fix order: gate `ExEffect::EditFile` and `QfCommand::FromFile` in the app layer
exactly like `embed.rs:204-224` (check + `resolve_under` when
`fs_restricted()`), and refuse `:cd` (or re-anchor the confinement root to a
fixed startup cwd) while the policy is active.

#### 2. LOW — RPC file-open can trigger remote grammar clone+compile+dlopen (the §3 deferred risk is reachable)

§3 ("Remote grammar compilation and dlopen", issue #314) says the risk "is not
remotely reachable today because the manifest is include_str! bundled". That
reachability half is no longer true: opening a file over nvim-api RPC
(`:e <path>` — the same ungated path as finding 1) calls `set_language_for_path`
(`apps/hjkl/src/app/mod.rs:922` from `build_slot`) → `request_for_path`
(`crates/hjkl-lang/src/lib.rs:121`) → slow path `load_async`
(`hjkl-lang/src/lib.rs:156-158`) → `GrammarLoader::load`
(`crates/hjkl-bonsai/src/runtime/loader.rs:123`) → `build_and_install`
(`loader.rs:153-204`): network clone (`source.rs:482/504`), system-compiler
compile (`compile.rs:90`), install, then `dlopen` of the built `.so` in process.
The manifest pins `git_url`/`git_rev` (no signature/hash — §3's design point),
so an RPC client can force fetch+compile+execute of the bundled grammars'
upstream code on demand, and can do so repeatedly (no rate/policy gate). The
correct trigger is the file-open path, not `:Anvil` (Anvil installs tools via
checksummed releases; it never touches grammars — the earlier draft attribution
was wrong, corrected here).

```
Repro: nvim_command(":e /tmp/foo.rs") on a host without the rust grammar installed
Expect: no network fetch in RPC mode (or an explicit policy refusal)
Actual: git clone + $CC compile + dlopen of the upstream tree-sitter-rust source
```

Fix: in `fs_restricted()`/RPC mode, resolve grammars via the on-disk tiers only
(`lookup_only` / `lookup_fresh`), never `load` — or gate `load_async` behind an
explicit opt-in flag.

### Cleared

- `:r` (read into buffer) IS gated: `check_fs_path` + `resolve_under` at
  `builtins.rs:169-184` — verified.
- `:w`/`saveas` write path IS gated: `save.rs:197` (`check_fs_path`) +
  `:205-217` (`resolve_under` + symlink-safe probe) — verified.
- Shell-out is disabled for non-TUI unless `--allow-shell` (`main.rs:529-531`,
  `policy::disable_shell`), and `:grep`/`:make` shell out through the same gate
  (`builtins.rs:132`) — verified.
- msgpack parsing of RPC params is hardened against malformed input
  (`nvim_api.rs` "malformed-input hardening" section, `as_array`/`param_str`
  error paths) — no panic-on-untrusted-input found in the walked range.
- Modeline values go through `set_by_name` validation (`modeline.rs:129-131`) —
  a hostile modeline cannot set arbitrary settings, only valid option values.

### Hardening

- `:cd` is a process-global side effect with no confinement interaction; a fixed
  startup-anchored root (or `:cd` refusal under `fs_restricted()`) would make
  the write-side `resolve_under` immovable.
- `nvim_buf_set_lines` + `:w <name>` + `:e <name>` can write a file with a
  `# vim: ts=0` modeline and then `:retab` to hit review finding §9.2's panic
  remotely — the two findings compose. Fixing the retab `.max(1)` closes it.

### Coverage

Entry points walked: `nvim_command`/`nvim_buf_set_lines`/`nvim_input`
(`nvim_api.rs` dispatch), embed-mode RPC (`embed.rs`), ex dispatch
(`ex_dispatch.rs`, `builtins.rs` read/write/quickfix/cd handlers), save path
(`save.rs`), policy (`policy.rs`), grammar loader (`hjkl-lang`,
`hjkl-bonsai/runtime/*`), shell-out gate. Not re-walked in this pass: the full
`nvim_api.rs` body outside the dispatch range, TUI input/render paths (no
untrusted input), `hjkl-app` config/filestate/undofile (covered by prior §8).
Platform-gated clipboard code (windows.rs / macos.rs) not compiled on this host.

Summary: 2 findings — 1 high, 1 low. Overall risk moderate-to-high for
`--nvim-api` deployments: the fs-confinement that RPC mode advertises is
bypassable on the read side outright and on the write side via `:cd`. Top fixes:
(1) gate `EditFile`/`FromFile` in the app layer like `embed.rs` does, (2) refuse
`:cd` under `fs_restricted()` or anchor the root at startup, (3) make the
grammar loader resolve on-disk-only in RPC mode.

## 11. 2026-08-06 tidy (whole-codebase cleanup sweep)

Clean tree on `main` → whole-codebase scope. clippy
(`--all-targets --all-features -D warnings`) is green, so the findings below are
what the compiler can't see: duplicated logic (the bulk), dead bindings hidden
behind `let _ =` suppressors, and dead items behind `#[allow(dead_code)]`. All
are behavior-preserving deletions/refactors; every finding verified by
re-reading both copies and their callers.

### Findings

#### 1. Identical `ExpandContext` builders — `apps/hjkl/src/app/prompt.rs:19` vs `ex_dispatch.rs:3193`

Textually identical (same alt-path lookup, same `cursor_word_tokens` call, same
`ExpandContext { …, cwd: None }`). prompt.rs's own comment admits it. Both
private, single caller each. Action: make `ex_dispatch::build_expand_context`
`pub(crate)`, delete `build_inline_expand_context`, call the shared one from
prompt.rs:502. Verified byte-identical (only the `super::` path differs).

#### 2. Leading-command-word extraction duplicated — `prompt.rs:339-362` vs `367-381`

`command_line_is_runnable` re-implements (range-prefix skip → trim → alnum/`_`
take-while) what `command_word_range` computes one method down. Unifying changes
behavior only for a leading token with non-ASCII alphanumerics (can never
resolve to an ex command — registry names are ASCII); the copies also disagree
(`is_alphanumeric` vs `is_ascii_alphanumeric`). Pick `is_ascii_alphanumeric`
deliberately. Verified: two copies differ in the char class.

#### 3. "Restore last search pattern" block duplicated verbatim — `prompt.rs:723-734` and `788-798`

Byte-identical `last_search()` → `Regex::new` → `set_search_pattern` block in
`cancel_search_prompt` and the `<C-f>` search-field branch. Verified identical
at both sites. Action: extract one helper.

#### 4. History-navigation block duplicated — `prompt.rs:553-580` and `743-774`

Save-current-on-first-nav + `history_prev/next` + `set_field_text` sequence
identical in command-field and search-field handlers. Verified. Action: extract
a shared `step_history` inner block; call sites keep their own history selection
and post-step calls.

#### 5. Dead `lines` trim block in two sibling setters — `crates/hjkl-engine/src/editor.rs:3739-3747` and `3766-3775`

Both `set_content` and `set_content_undoable` build a `Vec<String>`, trim
trailing empties, then `let _ = lines;` and
`replace_all(&mut self.buffer, text)` with the RAW text — `lines` is never read.
Verified: the block is inert in both functions. Action: delete the block +
suppressor from both. (If trailing-empty-line trimming was _intended_, that's a
separate correctness item — today it isn't happening.)

#### 6. Byte-identical rope helpers — `crates/hjkl-buffer/src/edit.rs:586-607` vs `buffer.rs:834,844`

`rope_line_str_locked` / `rope_line_content_end_locked` are exact copies of
`buffer::rope_line_str` / `rope_line_content_end` with identical signatures; the
doc's "takes a lock guard's rope by ref" rationale is false. edit.rs already
calls `crate::buffer::floor_char_boundary` at :606, so the "avoid re-importing"
excuse is moot. Single call site edit.rs:311. Action: call
`crate::buffer::rope_line_str`; delete both `_locked` fns.

#### 7. Five near-identical token-expansion blocks — `crates/hjkl-ex/src/expand.rs:143-195`

`expand_filename` repeats the same shape for `%`, `#`, `<cword>`, `<cWORD>`,
`<cfile>` (exact or `prefix:` → strip → `apply_modifiers(base, mods, cwd)`),
differing only in prefix and base source. Verified all five. Action: extract
`expand_one(base, token, prefix, cwd)` and call it five times.

#### 8. Dead tuple element — `crates/hjkl-ex/src/expand.rs:204-234, 278`

`extract_token` returns `(token, after)`; its only caller discards `after`
(`let _ = after;` — the caller advances by `token.len()`, exactly what `after`
represents). Verified at :267-278. Action: return only `token`.

#### 9. Dead binding + stale suppressor — `crates/hjkl-ex/src/listings.rs:100, 111`

`let back_len = back.len();` never read; `let _ = back_len; // used above` is
wrong. Verified. Action: delete both lines.

#### 10. Dead binding in let-else — `apps/hjkl/src/app/prompt.rs:394-400`

`command_completion_is_arg` destructures `(Some(range), Some(field))` then
discards `field`; the binding exists only to check `command_field.is_some()`.
Verified. Action: `(Some(range), _)` and drop the suppressor.

#### 11. Stale suppressor + stale comment — `crates/hjkl-engine/src/editor.rs:4370-4372`

`let _ = cur_top;` with a comment about mirroring to the textarea — but
`cur_top` IS used at :4366, and the textarea is documented as ripped out
(editor.rs:2463, :2551). Verified. Action: delete comment + suppressor.

#### 12. Unused tuple value — `crates/hjkl-app/src/picker_git.rs:1308-1312`

`name_len` computed in the destructure, immediately discarded; the fn recomputes
the char count as `name_char_end` at :1331. Verified. Action:
`let (is_head, kind, _) = …`.

#### 13. Four stale suppressors in explorer — `apps/hjkl/src/app/explorer.rs:940, 1056, 1668, 1742`

`let _ = X;` debris from the #151 Phase D refactor. Three values are still used
elsewhere (`new_win_id` → :906/:924; `win_id` → :994; `win_id` → :1584-1629), so
the suppressor is pure noise; `slot_idx` at :1742 (in `explorer_open_in_dir`) is
genuinely unused after the `explorer_slot_idx()` let-else. Verified. Action:
delete the three noise lines; rename :1742's binding to `_slot_idx` (keeping the
let-else guard) and delete the suppressor.

#### 14. Dead test helper — `apps/hjkl/src/nvim_api.rs:2565-2568`

`make_buf_param` carries `#[allow(dead_code)]` and has zero call sites (grep:
definition only). Verified. Action: delete fn + attribute.

#### 15. Language-name validation triplicated — `crates/hjkl-bonsai/src/highlighter.rs:1147-1152, 1353-1358, 1776-1781`

Three sibling functions repeat the identical
`!name.is_empty() && name.len() <= 64 && all(is_ascii_alphanumeric | - | _)`
guard around the same `injections.push((name, range))`. Verified all three.
Action: extract `valid_injection_lang(name)` + a small push helper.

#### 16. Minor / deliberate-keep (low priority)

- `apps/hjkl/src/app/mouse.rs:711-714` — `Tracker::reset` dead under
  `#[allow(dead_code)]`, comment says "Currently unused — the 500ms timeout
  handles natural resets." Author documented keeping it; delete only if that
  intent lapsed.
- `prompt.rs:187` vs `:493` — char→byte caret conversion duplicated; folds into
  finding 4's refactor.

### Coverage

Walked in detail:
`apps/hjkl/src/app/{prompt,ex_dispatch (3170-3244), explorer (cited regions), mouse (680-780), mod.rs (1700-1740), nvim_api (2530-2580)}`;
`crates/hjkl-ex/src/{expand,listings (90-130)}`;
`crates/hjkl-buffer/src/{edit (575-630), buffer (330-370, 830-860)}`;
`crates/hjkl-engine/src/{editor (2610-2625, 3725-3795, 4350-4395), buffer_impl, rope_util, types (1038-1050, 1540-1560)}`;
`crates/hjkl-app/src/picker_git (30-150, 1295-1350)`;
`crates/hjkl-bonsai/src/{theme (full), highlighter (cited regions)}`;
`crates/{hjkl-buffer-tui/src/render, hjkl-tabs, hjkl-xdg, hjkl-fuzzy}` (full).
Workspace-wide greps: every `pub trait` impl-count, every `#[allow(dead_code)]`
(44 sites), every `let _ =` suppressor (~30, read in context), every word-char
predicate, full-workspace clippy (green).

Checked and deliberately dropped: single-impl traits (`VimEditorExt`,
`EditorRatatuiExt`, `StyleResolver`, `StyleExt`, `bonsai::Theme`) — published
crate public-API seams with external consumers; `pos_to_position` /
`position_to_pos` (real grapheme-vs-char conversions); `rope_line_to_str`
(documented consolidation shim); `slots_mut` (documented test-only API);
clipboard/anvil `#[allow(dead_code)]` (platform-gated port code); cfg-gated
suppressors in `fs/open.rs`; documented no-op trait defaults.

Not reviewed in detail (GAP): `hjkl-vim` (~23k LOC), `hjkl-clipboard` (~14k,
mostly a port), `hjkl-anvil`, `hjkl-bonsai` highlighter body beyond cited
regions, `hjkl-css`, `hjkl-lsp`, `hjkl-form`, `hjkl-layout`,
`hjkl-compat-oracle`, `hjkl-picker/src`, the bodies of
`apps/hjkl/src/{render,embed,nvim_api,keymap_translate,completion,save,host}.rs`
and most `app/*` files, and the bulk of
`engine/{motions,editor,search,substitute}.rs`. Duplication there is possible
but unexamined.

## 12. 2026-08-06 perf (whole-workspace sweep)

Clean tree on `main` → whole workspace. The codebase has been through prior perf
passes (2026-08-04 review shipped ten findings; render/syntax/folds/
picker/completion caches are gen-keyed and windowed). Three findings remain;
every traced-but-clean path is listed under Coverage. All findings verified by
re-reading the cited lines and their callers.

### Findings — ranked by impact

#### 1. MEDIUM-HIGH — picker rebuilds the whole filtered list every frame (O(N) allocs + a regex compile per row)

While the picker is open, every frame materializes labels, match positions, and
styles for EVERY filtered entry — up to 500 for in-memory sources, but UNBOUNDED
for spawn sources (rg grep) — plus a ratatui `ListItem` with per-char `String`
allocations each.

Chain (all verified):

- `crates/hjkl-picker/src/picker.rs:471-484` `visible_entries` calls
  `source.label(idx)` + `source.label_match_positions(idx, query, &label)` per
  entry; `:489-499` `visible_entry_styles` calls `source.label(idx)` a SECOND
  time per entry. Nothing caches across frames.
- `crates/hjkl-picker/src/picker.rs:283-288` — spawn-mode sources keep ALL items
  in `filtered` (the 500 cap at `:357` applies only to the in-memory filter
  path), so N = the full grep match count.
- `crates/hjkl-picker/src/source/rg.rs:218-237` `label` takes the items `Mutex`
  and `format!`s a fresh String per call; `:315-364` `label_match_positions`
  takes the lock again, clones/truncates the text, COMPILES
  `regex::Regex::new(query)` per entry per frame (`:341-343`, the expensive
  part, ~tens of µs each), and allocates a char-offset Vec.
- `apps/hjkl/src/render.rs:2897-2898` calls `visible_entries()` /
  `visible_entry_styles()` unconditionally (after `p.refresh()` early-returns
  when nothing changed), and `:2904-2934` builds a `ListItem` for every entry —
  `:2916` `ch.to_string()` per char, `:2917-2921` a linear
  `styles.iter().find()` per char.

Why it matters: `apps/hjkl/src/app/event_loop.rs:487` — `self.picker.is_some()`
makes the frame TIME-ANIMATED, so the app repaints continuously on idle polls
(poll timeout ~16-30 ms per event_loop.rs:412-432 while the picker is up), not
just per keystroke. A 2000-match `rg` picker at ~60 fps is ~120k `label()`
builds and ~120k regex compiles per second on the main thread, all to draw ~30
visible rows.

Fix: window the list — materialize only the ~`list_area.height` rows around the
selection, passing the slice base as the ratatui `ListState` offset (the widget
scrolls by selection; only the offset needs to stay correct). Optionally cache
the materialized rows in `refresh()`, rebuilt only when query/count changed (the
gen-key pattern used everywhere else in this codebase), and hoist the
query→`Regex` compile out of the per-entry loop (compile once per query change,
reuse across entries and frames).

#### 2. LOW — explorer render filters the whole conceals list per visible row

`crates/hjkl-buffer-tui/src/render.rs:803-808` (and duplicate sites `:742-747`,
`:1193-1196`): per visible row,
`self.conceals.iter().filter(|c| c.row == doc_row).collect()` + sort —
O(visible_rows × conceals) per frame. Conceals exist only for the explorer slot,
one per non-root tree line (`apps/hjkl/src/app/explorer.rs:543-550`); a large
tree (thousands of nodes) with a ~50-row viewport costs ~50k-250k comparisons
plus a small Vec alloc per row, per frame, while the explorer is visible.

Fix: pre-index conceals by row once per frame (group into a
`HashMap<usize, Vec<&Conceal>>` or a row-sorted Vec + binary search) — the same
pattern the file already uses for `signs_by_row` (`render.rs:572-576`) and
`fold_index` (`:616`). Memory cost: one index build per frame, bounded by tree
size.

#### 3. LOW — `publishDiagnostics` materializes a full line String per diagnostic, twice for single-line ones

`apps/hjkl/src/app/lsp_glue.rs:487-503`: for every diagnostic in a publish,
`hjkl_buffer::rope_line_str(&rope, start_row)` and again for `end_row` build
full owned line Strings just to feed `wire_to_col` (`:114`, which only needs a
char/byte walk). The server re-publishes all of a file's diagnostics on every
edit; ~500 diagnostics → ~1000 line-String builds (plus a UTF-8/16 width scan of
the whole line each) per publish. Also: the slot match at `:456-465` calls
`std::env::current_dir()` per slot when filenames are relative — a syscall per
slot per publish (verified at lsp_glue.rs:461).

Fix: compute the column from the rope line directly — `rope.line(row)` is a
borrowed `RopeSlice`; iterate its chunks (or make `wire_to_col` generic over an
iterator of `&str`) instead of building the contiguous String. Skip the second
materialization when `start_row == end_row` (the common single-line case —
currently builds the same line twice). Cache cwd (or match against the
already-absolute stored path) in the slot lookup.

### Coverage

Traced and found clean (no findings): buffer render core
(`crates/hjkl-buffer-tui/src/render.rs` prefetch `:642-653`, `line_at` borrow
reuse `:659-665`, fold index `:616`, signs binary search `:572-576`/
`:1342-1343`, search-range frame cache `:1278-1289`, `Wrap::None` stack path
`:819-827`); `apps/hjkl/src/render.rs` (gutter/lnum memo, colorcolumn memo
`:845-856`, search-cache population `:831-840`, syntax recompute gated on
`(buffer_id, vp, height, dg)` at `app/mod.rs:1874-1887`, idle-draw skip
`event_loop.rs:483-503`, dirty-signature gen-cache `types.rs:600-632`, git signs
throttled + worker-threaded, blame async); syntax (`hjkl-syntax`
row_starts/dirty_gen caches, incremental parse, span cache extend; `hjkl-bonsai`
folds + injection-region memo); engine (search `SearchState::matches_for`
per-row gen cache `search.rs:488-507`, `buf_line_chars`/`rope_line_slice`
zero-alloc, undo rope Arc-clone snapshots, multicursor lens gated behind
`extra_selections.is_empty()` — known, backlog §1.3); picker internals
(`refresh()` lower-cache, scored-buf reuse); completion lower-cache; keymap
trie; which-key static tables; statusline; ex completion; diff alignment
gen-keyed; LSP codec framing; filestate/swap (cold, idle-gated); clipboard
threads; holler toasts; icons.

Not traced / could not settle without profiling: tree-sitter parse + highlight
per edit on very large files (native cost; caches confirmed, absolute numbers
need profiling); the conceals finding's exact per-frame cost in the explorer
(bounded above by rows × nodes; not measured); the picker finding's actual frame
rate while open (assumes the ~16-33 ms poll timeout holds while the picker is
up); platform-gated code (Windows/macOS clipboard, `hjkl-fs` Windows paths) —
